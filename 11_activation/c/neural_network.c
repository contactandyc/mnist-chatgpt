...
void printNetwork(NeuralNetwork* network) {
    for( int i=0; i<network->num_layers; i++ ) {
        Layer *layer = network->layers[i];
        printf( "biases(%d): ", i);
        printVector(layer->biases, layer->size);
        printf( "weights(%d): ", i );
        printMatrix(layer->weights, layer->size, layer->inputs_size);
    }
}
...
Layer* addDenseLayer(NeuralNetwork* network, int size, void (*activation_function)(float*, int), void (*activation_derivative)(float*, int), float** (*initialization_function)(int, int)) {
    ...
    layer->biases = (float*)ac_calloc(size * sizeof(float));
    layer->weights = initialization_function(inputs_size, size);  // Use the passed initialization function
...
float* forwardPass(NeuralNetwork* network, float* inputs) {
    ...
    for (int i = 0; i < network->num_layers; i++) {
        ...
        testMatrix(layer->weights, layer->size, layer->inputs_size);
        testVector(layer->outputs, layer->size);
...
void train(NeuralNetwork* network, Dataset* data, int epochs, float learning_rate) {
    ...
    float *output_error = output_layer->output_error;
    int output_size = output_layer->size;
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (int i = 0; i < data->num_samples; i++) {
            ...
            testVector(output, output_layer->size);
